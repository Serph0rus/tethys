...
static mut BOOTSTRAP_HEAP: [MaybeUninit<u8>; BOOTSTRAP_HEAP_SIZE] = [MaybeUninit::new(0); BOOTSTRAP_HEAP_SIZE];
static BOOTSTRAP_ALLOCATOR: linked_list_allocator::LockedHeap = linked_list_allocator::LockedHeap::empty();
...
unsafe impl GlobalAlloc for StubGlobalAllocator {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: core::alloc::Layout) {
        if unsafe {(BOOTSTRAP_HEAP.as_ptr() as *const u8 <= ptr) & (BOOTSTRAP_HEAP.as_ptr().wrapping_add(BOOTSTRAP_HEAP_SIZE) as *const u8 > ptr)} {
            unsafe {BOOTSTRAP_ALLOCATOR.dealloc(ptr, layout)};
        } else {
            match ALLOCATOR.lock().as_ref() {
                Some(allocator) => unsafe {allocator.dealloc(ptr, layout)},
                None => panic!("Memory deallocation attempted on region not within bootstrap heap, before initialising system allocator!"),
            }
        }
    }
}
pub fn bootstrap_initialise(_boot_info: &mut bootloader_api::BootInfo) {
    BOOTSTRAP_ALLOCATOR.lock().init_from_slice(unsafe {&mut BOOTSTRAP_HEAP});
}